...
func newConfig() config {
	c := config{
		...
		clientIP:          !BoolEnv(envClientIPHeaderDisabled, false),
		queryString:       !BoolEnv(envQueryStringDisabled, false),
	...
	if s, ok := os.LookupEnv(envQueryStringRegexp); !ok {
		...
		log.Printf("%s is set but empty. Query string obfuscation will be disabled.", envQueryStringRegexp)
	} else if r, err := regexp.Compile(s); err == nil {
		c.queryStringRegexp = r
	} else {
		log.Printf("Could not compile regexp from %s. Using default regexp instead.", envQueryStringRegexp)
	}
	return c
}
